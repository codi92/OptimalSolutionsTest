pipeline {
       agent any
		...
		stage('Config containers') {
			...
			do
			ISOUR=`docker ps -a | grep $OUTPUT |awk '$2 ~ "_nginx" {print $1}'`
			container_name=`docker ps |grep $ISOUR|awk '{system("echo "$NF" ")}'`
			container_hostname=`echodocker exec -i $ISOUR  cat /etc/hostname`
			...
			echo "$ISOUR" |xargs --no-run-if-empty echo "<head><title>It is the app</title><style>.content {max-width: 500px;margin: auto;padding: 10px;}</style></head><body><div class="content"><h1> Hello World <br><h2> This is the $container_name container <br><h2> his hostname is : $container_hostname <br><h2> his ip is : $container_ip <br></div></body>">tmp
			echo "$ISOUR" |xargs --no-run-if-empty docker cp tmp $ISOUR:/var/www/app.slajnev.tk/public/index.html
			done
			'''
				}
			}				
